...
static void adjustAccelData(float *accel_vec, float *quat)
{
...
void UpdateState(float *a, float *g, float *m, float *q, float delta_time_sec, float *euler_angles)
{
    ...
    // Yaw
    euler_angles[0] = atan2(2.0f * (q[1] * q[2] + q[0] * q[3]), q[0] * q[0] + q[1] * q[1] - q[2] * q[2] - q[3] * q[3]) - DECLINATION;

    // Roll
    euler_angles[1] = -atan2(-2.0f * (q[0] * q[1] + q[2] * q[3]), -1.0f * (q[0] * q[0] - q[1] * q[1] - q[2] * q[2] + q[3] * q[3]));

    // Pitch
    euler_angles[2] = -asin(2.0f * (q[1] * q[3] - q[0] * q[2]));
}
...
void MadgwickQuaternionUpdate(float *a, float *g, float *m, float *q, float delta_time_sec)
{
    ...
    const float degreesToRad = (M_PI) / 180.0;

    float gx = g[0] * degreesToRad; // Pass gyro rate as rad/s
    float gy = g[1] * degreesToRad;
    float gz = g[2] * degreesToRad;
    ...
    q1 += qDot1 * delta_time_sec;
    q2 += qDot2 * delta_time_sec;
    q3 += qDot3 * delta_time_sec;
    q4 += qDot4 * delta_time_sec;
